...
BEGIN
    ...
    v_userId := public.get_auth_user_id(); -- Ahora devuelve NULL si no hay auth.uid()
    
    -- Definir acción y capturar IDs
    ...
    -- Si no hay usuario, marcamos como acción de sistema en los detalles
    IF v_userId IS NULL THEN
        v_details := v_details || jsonb_build_object('executed_by', 'SYSTEM_OR_MIGRATION');
    END IF;

    -- Si no pudimos extraer el companyId, caemos en el del usuario (si existe)
    IF v_companyId IS NULL AND v_userId IS NOT NULL THEN
        v_companyId := public.get_auth_user_company_id();
    END IF;

    -- Insertar log
    -- userId es nullable, por lo que si v_userId es NULL, la FK no se valida (pasa)
