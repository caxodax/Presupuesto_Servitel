  ...
  return (
    <div className="flex flex-col gap-6 pb-12 h-fit min-h-full">
      ...
      <div className="flex items-center gap-3 bg-zinc-100 dark:bg-zinc-900/50 p-1.5 rounded-lg border border-zinc-200 dark:border-zinc-800 w-fit">
          <Link href="/dashboard/auditoria?actionFilter=" className={`px-4 py-1.5 rounded-md text-xs font-semibold transition-colors ${actionFilter === '' ? 'bg-white dark:bg-zinc-800 shadow-sm text-foreground' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>
              Todas Operaciones
          </Link>
          <Link href="/dashboard/auditoria?actionFilter=CREATE_INVOICE" className={`px-4 py-1.5 rounded-md text-xs font-semibold transition-colors ${actionFilter === 'CREATE_INVOICE' ? 'bg-white dark:bg-zinc-800 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>
              Registro Carga De Facturas (Egresos)
          </Link>
          <Link href="/dashboard/auditoria?actionFilter=ADJUST_BUDGET" className={`px-4 py-1.5 rounded-md text-xs font-semibold transition-colors ${actionFilter === 'ADJUST_BUDGET' ? 'bg-white dark:bg-zinc-800 shadow-sm text-rose-600 dark:text-rose-400' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>
         ...
         {metadata.totalPages > 1 && (
            <div className="border-t border-zinc-200 dark:border-zinc-800/50 p-4 bg-zinc-50/50 dark:bg-zinc-900/50 flex items-center justify-between">
               ...
               <div className="flex gap-2">
                  <Link 
                     href={`/dashboard/auditoria?page=${metadata.page - 1}${actionFilter ? `&actionFilter=${actionFilter}` : ''}`}
