           ...
           <table className="w-full text-sm text-left">
             <thead className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800/50 text-zinc-500 font-medium">
               <tr>
                 ...
                 <th className="px-6 py-4 text-center">Filtro Estado</th>
               </tr>
             </thead>
             <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/50">
               {invoices.map(inv => (
                 <tr key={inv.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/10 transition-colors group cursor-pointer relative">
                   ...
                   <td className="px-6 py-4">
                       <span className="text-muted-foreground">{inv.allocation.category.name} <span className="text-zinc-600 dark:text-zinc-400">·</span> {inv.allocation.budget.branch.name}</span>
                       <div className="text-[11px] text-zinc-400 uppercase tracking-widest font-semibold mt-0.5">{inv.allocation.budget.name}</div>
                   </td>
                   ...
                   <td className="px-6 py-4 text-center">
                       <span className="inline-flex items-center px-2 py-0.5 rounded-[4px] text-[11px] font-bold tracking-wider bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">
                           {inv.status}
                       </span>
                   </td>
                 </tr>
