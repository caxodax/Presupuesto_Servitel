    ...
    return (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="group relative overflow-hidden rounded-2xl border border-emerald-100/50 dark:border-emerald-900/30 bg-white/70 dark:bg-zinc-900/40 p-6 shadow-[0_8px_32px_rgba(0,0,0,0.05)] backdrop-blur-xl transition-all duration-500 hover:shadow-[0_12px_44px_rgba(16,185,129,0.1)] hover:-translate-y-1">
                ...
                <div className="flex justify-between items-start mb-6">
                    <div className="flex flex-col gap-1">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">Ingresos Totales</h3>
                        <p className="text-xs text-zinc-500 font-medium tracking-tight">Capital Entrante</p>
                    </div>
                    <div className="h-10 w-10 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-emerald-500/20 group-hover:scale-110 transition-transform">
                ...
                <div className="mt-6 flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.1em]">Flujo de Caja Activo</span>
                </div>
            </div>
            ...
            <div className="group relative overflow-hidden rounded-2xl border border-white/20 dark:border-zinc-800/50 bg-white/70 dark:bg-zinc-900/40 p-6 shadow-[0_8px_32px_rgba(0,0,0,0.05)] backdrop-blur-xl transition-all duration-500 hover:shadow-[0_12px_44px_rgba(0,0,0,0.1)] hover:-translate-y-1">
                ...
                <div className="flex justify-between items-start mb-6">
                    <div className="flex flex-col gap-1">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Presupuesto Disponible</h3>
                        <p className="text-xs text-zinc-500 font-medium tracking-tight">Disponibilidad de Giro</p>
                    </div>
                    <div className="h-10 w-10 bg-zinc-100 dark:bg-zinc-800 rounded-xl flex items-center justify-center text-zinc-400 group-hover:scale-110 transition-transform">
                        <Layers className="w-5 h-5" />
                    </div>
                </div>
                <div className="text-3xl font-black tracking-tighter text-zinc-400">
                   ${Math.max(kpis.availableCapacity, 0).toLocaleString()}
                </div>
                <div className="mt-6 flex items-center gap-2">
                    <span className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.1em]">Reserva Operativa</span>
                </div>
            </div>
            ...
            <div className={`group relative overflow-hidden rounded-2xl border transition-all duration-500 backdrop-blur-xl p-6 shadow-[0_8px_32px_rgba(0,0,0,0.05)] hover:-translate-y-1
                 ${kpis.netBalance >= 0 
                     ? 'border-indigo-100 dark:border-indigo-900/30 bg-white/70 dark:bg-zinc-900/40 hover:shadow-[0_12px_44px_rgba(99,102,241,0.1)]' 
                     : 'border-rose-100 dark:border-rose-900/30 bg-rose-50/30 dark:bg-rose-950/10 hover:shadow-[0_12px_44px_rgba(244,63,94,0.1)]'}`
            }>
                <div className="absolute top-0 right-0 -mr-4 -mt-4 h-24 w-24 rounded-full bg-indigo-500/5 blur-3xl" />
                <div className="flex justify-between items-start mb-6">
                    <div className="flex flex-col gap-1">
                        <h3 className={`text-[10px] font-black uppercase tracking-[0.2em] ${kpis.netBalance >= 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-rose-600'}`}>
                           Balance Neto
                        </h3>
                        <p className="text-xs text-zinc-500 font-medium tracking-tight">Utilidad Bruta / Pérdida</p>
                    </div>
                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center text-white shadow-lg transition-transform group-hover:scale-110 ${kpis.netBalance >= 0 ? 'bg-gradient-to-br from-indigo-500 to-indigo-700 shadow-indigo-500/20' : 'bg-gradient-to-br from-rose-500 to-rose-700 shadow-rose-500/20'}`}>
                        <TrendingUp className={`w-5 h-5 ${kpis.netBalance < 0 ? 'rotate-180' : ''}`} />
                    </div>
                </div>
                <div className={`text-3xl font-black tracking-tighter ${kpis.netBalance >= 0 ? 'text-indigo-600 dark:text-indigo-400' : 'text-rose-600 dark:text-rose-400'}`}>
                   ${kpis.netBalance.toLocaleString()}
                </div>
                <div className="mt-6 flex items-center gap-2">
                    <span className={`text-[10px] font-black uppercase tracking-[0.1em] ${kpis.netBalance >= 0 ? 'text-indigo-600' : 'text-rose-600'}`}>
                        {kpis.netBalance >= 0 ? 'Superávit Financiero' : 'Déficit de Caja'}
                    </span>
                </div>
            </div>
        </div>
