...
export function IncomeModal({ 
    ...
    const {
        ...
        defaultValues: mode === 'edit' ? {
            ...income,
            date: income?.date && !isNaN(new Date(income.date).getTime()) ? format(new Date(income.date.includes("T") ? income.date : income.date + "T00:00:00"), "yyyy-MM-dd") : format(new Date(), "yyyy-MM-dd"),
            incomeId: income.id,
            accountId: income.accountId || null,
            companyAccountId: income.companyAccountId || null
        } : {
    ...
    const watchCompanyAccountId = watch("companyAccountId" as any)
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <AccountSelector 
                                    ...
                                    defaultValue={watchCompanyAccountId || watchAccountId}
                                    onSelect={(id) => {
                                        setValue("companyAccountId" as any, id)
                                        setValue("globalAccountId" as any, null)
                                    }}
                                    type="INCOME"
                                    isExecutable={true}
