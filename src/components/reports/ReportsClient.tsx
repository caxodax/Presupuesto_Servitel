...
export function ReportsClient({ companies, branches, categories, businessGroups, userRole, initialCompanyId }: ReportsClientProps) {
    ...
    const [expandedRows, setExpandedRows] = useState<Record<string, boolean>>({ "1": true, "4": true, "5": true, "6": true }) // Expandir niveles principales por defecto
    const [isMounted, setIsMounted] = useState(false)
    const [filters, setFilters] = useState<Filters>({
        startDate: '2026-01-01', // Fixed string for initial SSR consistency
        endDate: '2026-12-31',
    ...
    useEffect(() => {
        setIsMounted(true)
        // Adjust dates to real "today" once on client
        setFilters(prev => ({
            ...prev,
    ...
    if (!isMounted || (!reportData && isPending)) return <ReportsSkeleton />

    const balance = reportData ? reportData.summary.totalIncome - reportData.summary.totalExpense : 0
    const isProfit = balance >= 0

    return (
        <div className="flex flex-col gap-8 animate-in fade-in duration-700" suppressHydrationWarning>
            ...
            {/* Premium Tabs */}
            <div className="flex flex-wrap items-center gap-2 p-1.5 bg-zinc-100 dark:bg-zinc-800/50 rounded-2xl w-fit self-center lg:self-start">
                <button 
                ...
                <button 
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'hierarchical' ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-md' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}
                >
                    <PieIcon className="w-4 h-4" /> Plan de Cuentas
                </button>
                <button 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="bg-white dark:bg-zinc-900 p-8 rounded-[40px] border border-zinc-200/60 dark:border-zinc-800 shadow-[0_8px_30px_rgb(0,0,0,0.02)] relative overflow-hidden group hover:border-rose-500/30 transition-all">
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="text-3xl font-black tracking-tighter text-zinc-900 dark:text-white mt-1">
                                    ${reportData?.summary.totalOut.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </div>
                                <div className="flex gap-2 mt-2">
                                    <span className="text-[8px] font-bold text-zinc-400 uppercase">Costos: ${reportData?.summary.totalCost.toLocaleString()}</span>
                                    <span className="text-[8px] font-bold text-zinc-400 uppercase">Gastos: ${reportData?.summary.totalExpense.toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                ...
                <div className="animate-in slide-in-from-right-4 duration-500 flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-emerald-500/5 border border-emerald-500/10 p-6 rounded-[32px] flex items-center justify-between">
                            <div>
                                ...
                                <h4 className="text-2xl font-black text-emerald-700">
                                    ${((reportData?.summary.totalIncome || 0) - (reportData?.summary.totalCost || 0)).toLocaleString()}
                                </h4>
                            </div>
                            <TrendingUp className="w-8 h-8 text-emerald-500/20" />
                        </div>
                        <div className="bg-rose-500/5 border border-rose-500/10 p-6 rounded-[32px] flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-black uppercase text-rose-600 tracking-widest mb-1">Carga Operativa (Gastos)</p>
                                <h4 className="text-2xl font-black text-rose-700">
                                    ${(reportData?.summary.totalExpense || 0).toLocaleString()}
                                </h4>
                            </div>
                        ...
                        <div className="bg-indigo-500/5 border border-indigo-500/10 p-6 rounded-[32px] flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-black uppercase text-indigo-600 tracking-widest mb-1">Utilidad Neta (EBITDA)</p>
                                <h4 className="text-2xl font-black text-indigo-700">
                                    ${(reportData?.summary.utility || 0).toLocaleString()}
                                </h4>
                            </div>
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-[48px] border border-zinc-200/60 dark:border-zinc-800 shadow-sm overflow-hidden">
                        <div className="p-8 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                            <div>
                                <h3 className="text-lg font-black tracking-tighter uppercase">Estado de Resultados Detallado</h3>
                                <p className="text-[10px] text-zinc-400 font-black uppercase tracking-widest mt-1">Análisis por Niveles de Cuenta · Periodo Seleccionado</p>
                            </div>
                        </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-zinc-50 dark:bg-zinc-800/50">
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-zinc-400">Código / Cuenta</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-zinc-400 text-center">Tipo</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-indigo-500 text-right">Presupuesto</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-rose-500 text-right">Ejecutado</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-emerald-500 text-right">Disponible</th>
                                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-zinc-400 text-center">% Ejecución</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                                    {(reportData?.tree || []).filter((row: any) => {
                                        if (row.level === 1) return true;
                                        // Mostrar si el padre está expandido
                                        const parentCode = row.code.substring(0, row.code.lastIndexOf('.'));
                                        const parentParts = row.code.split('.');
                                        // Si tiene padre (ej: 5.1), verificar si su padre (5) está expandido
                                        let currentCode = "";
                                        for (let j = 0; j < parentParts.length - 1; j++) {
                                            currentCode += (j > 0 ? "." : "") + parentParts[j];
                                            if (expandedRows[currentCode] === false) return false;
                                        }
                                        return true;
                                    }).map((row: any, i: number) => {
                                        const isMainCategory = row.level === 1;
                                        const isSubCategory = row.level === 2;
                                        const isExpanded = expandedRows[row.code] !== false;
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className="px-8 py-4">
                                                    <div className="flex items-center gap-3" style={{ paddingLeft: `${(row.level - 1) * 24}px` }}>
                                                        ...
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className={`text-xs ${isMainCategory ? 'font-black uppercase tracking-tight text-zinc-900 dark:text-white' : isSubCategory ? 'font-bold text-zinc-800 dark:text-zinc-200' : 'font-medium text-zinc-600 dark:text-zinc-400'}`}>
                                                                {row.name}
                                                            </span>
                                                        </div>
                                                    </div>
                                                ...
                                                <td className="px-8 py-4 text-right">
                                                    <div className="flex flex-col">
                                                        <span className={`text-xs font-black ${isMainCategory ? 'text-indigo-600' : 'text-zinc-900 dark:text-zinc-100'}`}>
                                                            ${row.budget.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                                        </span>
                                                        {row.budget === 0 && <span className="text-[8px] text-zinc-400 font-bold uppercase">Sin asignar</span>}
                                                    </div>
                                                </td>
                                                <td className="px-8 py-4 text-right">
                                                ...
                                                <td className="px-8 py-4 text-right">
                                                    <span className={`text-xs font-black ${row.available >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
